...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleWordClick(word, wordBlock, correctPair) {
        ...
        if (score >= 10) {
            ...
            showCongratulations(); // Поздравляем при достижении 10 очков
            clearInterval(timerInterval); // Останавливаем таймер
        }
    }
    ...
    function nextTask() {
        let task;
        // Если score <= 5, выбираем вопрос из массива tasks (1-5 уровень)
        if (score < 5) {
            task = getRandomTask(tasks);
        } else if (score < 10) {
            task = getRandomTask(imageTasks);  // Для уровня 6-10 из imageTasks
        }
        resetTimer();  // Сброс таймера при переходе к новому вопросу
        displayTask(task);
    }
    ...
    function resumeGame() {
        if (modal.style.display !== 'flex' && retryModal.style.display !== 'flex') {
